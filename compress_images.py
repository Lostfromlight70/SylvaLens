...
# Configuration
CATEGORIES = ['Weddings', 'Portraits', 'Sports', 'Ceremonies', 'Pre weddings', 'Creative']
MAX_WIDTH = 1200
MAX_HEIGHT = 1200
JPEG_QUALITY = 85
PNG_QUALITY = 85
    ...
    for category in CATEGORIES:
        ...
        for image_file in image_files:
            ...
            if compress_image(str(image_file), str(jpg_path), format='JPEG', quality=JPEG_QUALITY):
                ...
                print(f"  ✓ {image_file.name}: {original_size/1024:.1f}KB → {new_size/1024:.1f}KB ({reduction:.1f}% reduction)")
                processed_count += 1
